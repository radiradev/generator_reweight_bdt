...
class BDTReweighter {
 ...
 public :
  ...
  virtual float predict(std::vector<float> data, int pred_margin) = 0;

  float GetWeight(std::vector<float> data, int pred_margin){ return multiplier*predict(data, pred_margin); }
